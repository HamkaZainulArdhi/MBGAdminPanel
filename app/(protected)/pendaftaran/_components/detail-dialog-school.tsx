...
import { School, Vendor } from "@/lib/types/registration";
...
export function SchoolDetailDialog({
  ...
  const hasDocuments =
    data.photoSK ||
    data.photoSuratAktif ||
    data.photoStempel ||
    data.photoFotoSekolah;
      ...
      <div className="space-y-6 min-w-0">
        <Section title="Informasi Sekolah">
          <dl className="grid grid-cols-2 gap-x-6 gap-y-4">
            ...
            <Row
              label="Porsi"
              value={data.porsi != null ? `${data.porsi} siswa` : undefined}
            />
          ...
          <Section title="Kontak (PIC)">
            <dl className="grid grid-cols-2 gap-x-6 gap-y-4">
              <Row
                ...
                value={
                  data.picName ||
                  (typeof data.pic === "object" ? data.pic?.nama : data.pic)
                }
                fullWidth
              />
              <Row
                ...
                value={
                  data.picPosition ||
                  (typeof data.pic === "object" ? data.pic?.jabatan : undefined)
                }
              />
              <Row
                label="No. Telepon"
                value={
                  data.picPhone ||
                  (typeof data.pic === "object" ? data.pic?.hp : undefined)
                }
              />
            </dl>
          </Section>
        ...
        {(data.assignedVendorEmail || data.assignedDriverId) && (
          <Section title="Assignment (Legacy)">
            <dl className="grid grid-cols-1 gap-y-4">
              {data.assignedVendorEmail && (
                <Row
                  label="Vendor Email"
                  value={data.assignedVendorEmail}
                  mono
                />
              )}
              {data.assignedDriverId && (
                <Row label="Driver ID" value={data.assignedDriverId} mono />
              )}
            </dl>
          </Section>
        )}
        ...
        {onVendorSelected && (
          <Section title="Penugasan Vendor">
            <VendorSelect
              ...
              label="Pilih Vendor"
              placeholder="Tentukan vendor untuk sekolah ini..."
              disabled={isAssigningVendor || !data.isApproved}
              isLoading={isAssigningVendor}
              withDriversOnly={true}
            />
            {!data.isApproved && (
              <p className="text-xs text-muted-foreground/60 mt-2">
                ⓘ Sekolah harus disetujui terlebih dahulu sebelum assign ke
                vendor.
              </p>
            )}
          </Section>
